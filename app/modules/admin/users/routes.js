...
router.get('/addstock/:id', authMiddleware.hasAuthAdmin, (req, res) => {
    db.query('SELECT * FROM tblbloodcomponent', (err, results, fields) => {
        if (err) console.log(err);
        return res.render('admin/users/views/addstock', { tblbloodcomponent: results });
    });
});
router.post('/addstock/:id', authMiddleware.hasAuthAdmin, (req, res) => {
    console.log('here');
    console.log(req.params.id);
    db.query(`INSERT INTO \`tblbloodstock\` (  \`intBLOODBANK_ID\`, \`intBLOODCOMPONENT_ID\`, \`dblBLOODSTOCK_PRICE\`, \`strBLOODSTOCK_AP\`, \`strBLOODSTOCK_AN\`, \`strBLOODSTOCK_BP\`, \`strBLOODSTOCK_BN\`, \`strBLOODSTOCK_ABP\`, \`strBLOODSTOCK_ABN\`, \`strBLOODSTOCK_OP\`, \`strBLOODSTOCK_ON\`)
