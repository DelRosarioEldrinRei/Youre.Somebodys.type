...
router.get('/', authMiddleware.hasAuthMember, (req, res) => {
    db.query(`SELECT * FROM reserve, tblperson, tblaccounts WHERE tblaccounts.intACCOUNT_ID=${req.session.member.intACCOUNT_ID} AND tblperson.intACCOUNT_ID=tblaccounts.intACCOUNT_ID AND tblperson.intPERSON_ID=reserve.intPERSON_ID`, (err, results, fields) => {
        if (err) throw err;
...
router.post('/:id', authMiddleware.hasAuthMember, (req, res) => {
    ...
    db.query(`INSERT INTO \`tblreservation\` ( \`intACCOUNT_ID\`, \`intBLOODBANK_ID\`, \`intBLOODTYPE_ID\`, \`intBLOODCOMPONENT_ID\`, \`strQUANTITY\`)
